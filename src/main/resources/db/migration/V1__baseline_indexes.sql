...
-- CREATE INDEX IF NOT EXISTS idx_createorder_status ON createorders (status);

-- Created date (pagination/sorting by date)
-- CREATE INDEX IF NOT EXISTS idx_createorder_created_at ON createorders (created_at DESC);

-- Composite: status + created_at (covers most list queries)
-- CREATE INDEX IF NOT EXISTS idx_createorder_status_created
--     ON createorders (status, created_at DESC);

-- Foreign key index (prevents full-table scan on joins)
-- CREATE INDEX IF NOT EXISTS idx_createorder_user_id ON createorders (user_id);
