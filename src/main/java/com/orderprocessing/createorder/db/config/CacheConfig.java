package com.orderprocessing.createorder.db.config;
 ...
 *
 * Usage in service:
 *   @Cacheable(value = "createorderCache", key = "#id")
 *   @CacheEvict(value = "createorderCache", key = "#id")
 *   @CachePut(value  = "createorderCache", key = "#result.id")
 */
@Configuration
@EnableCaching
public class CacheConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory factory) {
        ...
        // ── createorder specific caches ─────────────────────────────────────
        caches.put("createorderCache",       defaultConfig().entryTtl(MEDIUM));
        caches.put("createorderListCache",   defaultConfig().entryTtl(SHORT));
        caches.put("createorderCountCache",  defaultConfig().entryTtl(SHORT));
        caches.put("createorderRefCache",    defaultConfig().entryTtl(LONG));
